...
- `toml-tidy` now normalizes every non-empty, single-line array to one space inside its brackets, including nested arrays.
- Multi-line array layout remains unchanged.
- Trunk now runs Ruff at the version in `uv.lock`.

### Security

- GitHub Actions checkout steps no longer persist credentials.
- TruffleHog exclusions now target only the three test signatures that produce false positives.

## [v0.4.1] — 2026-08-16

### Fixed

- `--in-place` dropped a macOS extended ACL from its target, which v0.4.0 introduced by replacing the file with a new inode that `shutil.copystat` cannot carry an ACL onto. A target holding one now takes the same rewrite-in-place path as a hard-linked target, so its entries survive the write; files without an ACL are still replaced atomically. A POSIX platform that is neither Linux nor macOS keeps the inode unconditionally rather than assume an ACL it cannot inspect would survive. On Linux, where the metadata copy does carry a POSIX ACL, the copy is confirmed on the replacement before it is put in place — the standard library swallows a refused attribute write, so it can report success having carried nothing.
- `--in-place` could **widen** a file's access, also since v0.4.0: the replacement is written in the target's own directory, so an inheritable ACL entry on that directory landed on it, and a `0600` file with no ACL of its own came back granting `everyone` read. The replacement's ACL is now compared against the target's in both directions, and any difference — gained or lost — keeps the original inode instead.
- `--in-place` cleared the set-user-ID and set-group-ID bits of any target it rewrote in place rather than replaced — since v0.4.0 for a hard-linked target, and on Windows. Such a target is now refused with exit `2` and left untouched instead. Writing clears those bits from its first byte, and restoring them needs ownership the writer need not have; for set-group-ID, a `chmod` by a caller outside the file's group clears the bit while reporting success, so even testing whether a restore would work destroys what it asks about. Ordinary set-ID files still sort normally, because the atomic replacement carries their mode across intact — only one that also has a hard link or an ACL, or is on Windows, is declined.
